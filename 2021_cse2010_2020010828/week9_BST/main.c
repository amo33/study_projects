...
int compareTwoTree(Node* t1, Node* t2); // 동일한 트리인지 비교 : 동일 == 1 아니면 0
...
void deleteTreeNode(Node** p, int value)
{
    ...
    else
    {
        ...
        while(childNode->rightChild != NULL){
        ...
        if(parentNode == NULL){
            ...
            succNode = searchTreeParentNode(delNode, childNode->data);
            succNode->rightChild = succNode->rightChild->leftChild;
            childNode->leftChild = delNode->leftChild;
            childNode->rightChild = delNode->rightChild;
            ...
            *p=childNode;
            ...
            succNode = searchTreeParentNode(delNode, childNode->data);
            succNode->leftChild = succNode->leftChild->rightChild;
            childNode->leftChild = delNode->leftChild;
            childNode->rightChild = delNode->rightChild;
...
void copyTreeNode(Node* src, Node** dst)
{
    ...
    Node* tempNode;
    tempNode = src;
    insertTreeNode(dst, tempNode->data);
...
int compareTwoTree(Node* t1, Node* t2)
{
    ...
    if(t1 == NULL && t2 == NULL && countT2 == countT1){
        return 1;
    }
    if(t1->data != t2->data){
        return -1;
