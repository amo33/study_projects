...
class StudentDetail(APIView):
    def get(self, response, student_id):
        model = Student.objects.get(student_id=student_id)
        serializer = StudentSerializer(model)
        return Response(serializer.data)

    def put(self, response, student_id):

        serializer = Student.objects.get(student_id=student_id)
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status =status.HTTP_404_BAD_REQUEST)
    def delete(self, respone, student_id):
        serializer = Student.objects.get(student_id=student_id)
        serializer.delete()
