...
# 다익스트라의 목적은 경로를 해당 노드에 업데이트하는데, 제일 작은 값을 구하고자 항상 작은 가중치의 경로를 픽해서 찾는것이다.
def networkDelayTime( times, n: int, k: int) -> int:
    time_map = collections.defaultdict(list)
    for source, end, weight in times:
        time_map[source].append([end,weight])
    print(time_map)
    ...
    def bfs(val):
        ...
        while queue:
            ...
            while time_map[node]:
                ...
                dest, weight =time_map[node].pop(0)
                print(dest, weight)
                visited[dest]= min(visited[dest], visited[node]+weight)
                print(visited)
                heapq.heappush(queue, (visited[dest],dest))
